(function () {
  ...
  const createError = (errorType, {asarPath, filePath, callback} = {}) => {
    let error
    if (!errors.includes(errorType)) throw new Error('invalid error type')
    if (errorType === 'NOT_FOUND') {
      error = new Error(`ENOENT, ${filePath} not found in ${asarPath}`)
      error.code = 'ENOENT'
      error.errno = -2
    } else if (errorType === 'NOT_DIR') {
      error = new Error('ENOTDIR, not a directory')
      error.code = 'ENOTDIR'
      error.errno = -20
    } else if (errorType === 'NO_ACCESS') {
      error = new Error(`EACCES: permission denied, access '${filePath}'`)
      error.code = 'EACCES'
      error.errno = -13
    } else if (errorType === 'INVALID_ARCHIVE') {
  ...
  const overrideAPI = function (module, name, arg = 0) {
    ...
    module[name] = function () {
      ...
      const archive = getOrCreateArchive(asarPath)
      if (!archive) createError('INVALID_ARCHIVE', {asarPath, callback})

      const newPath = archive.copyFileOut(filePath)
      if (!newPath) createError('NOT_FOUND', {asarPath, filePath, callback})
